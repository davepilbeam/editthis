...
urlStr: function(a){ var g = this.toString();if(!a){ g = g.replace(/\-/g,' ').replace(/\&amp;/g,'&').replace(/;/g,':').replace(/#/g,'?').replace(/\^/g,"'").replace(/\s\s/g,'-').replace(/\~/g,'/'); } else { g = g.replace(/[^a-zÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ-ÃÂÃÂÃÂÃÂÃÂÃÂÃÂÃÂ¿0-9:\?\'\/\-\s_\&]/gi,'').replace(/:/g,';').replace(/\?/g,'#').replace(/'/g,'^').replace(/^\s*/,'').replace(/\s*$/,'').replace(/\-/g,'--').replace(/\//g,'~').replace(/ /g,'-'); }return g; },
pageoutStr: function(){ return this.toString().replace(/\.html$/,'').replace(/:/g,';').replace(/\^/g,"'").replace(/#/g,'?').replace(/\-\-/g,'==').replace(/\-/g,' ').replace(/==/g,'-'); },
...
sf = function(sfa,sfb,sfc){ if(sfb){sk = sfb.getProperty('data-menu');console.log('sp = ',sp,' sk = ',sk);if(sk){sk = sk.replace(/\.(0|00)$/,'');sk+=sfa['menu'];sfb.setProperty('data-menu',sk);if( sk.test(/\.(0|00)$/) ){if(sk.test(/\.00$/)){sg = 'hidemappage';sfb.getParent('form.sendable').removeClass('hidepage');}sfb.getParent('form.sendable').addClass(sg);sfb.removeClass('showpage').addClass('mhidepage');} else {sfb.getParent('form.sendable').unclassMe('hidepage hidemappage');sfb.addClass('showpage').removeClass('mhidepage');}}}if(sfc &&sfa['label']){ sfc.set('html',sfa['label']); }return sfa['html']; };
if(sr == 'OK'){ if( sa['html'] ){ 
