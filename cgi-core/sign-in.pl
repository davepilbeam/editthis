...
our %signinfields = (defined %defs::signinfields)?%defs::signinfields:undef;
...
###sign_respond("Debug 1 from $invoker:",{ 'msg' => "Request received by server: $ENV{'SERVER_ADDR'} remote address: $ENV{'REMOTE_ADDR'} $serverenv == $serverip signin: $signin emailaddr: $emailaddr loginuser: $loginuser loginpass: $loginpass $debug" },"okarea");
...
if( $loginuser eq $signinfields{'masteruser'} && $loginpass eq $signinfields{'masterpass'} ){ sign_get_destination($baseview.$signinfields{'dest'},$loginuser,$loginpass); } else { sign_respond("Request denied:",{ 'msg' => "Invalid form information from $ENV{'REMOTE_ADDR'}. Please refresh page and try again." },"errorarea"); }

} elsif( defined $signin && defined $emailaddr ){ #email=admin@thatsthat.co.uk&name=pecreative

my @signup = ( $emailaddr );
my @fs = ( ['request',$signinfields{'useremail'}],['name',$signin],['email',$emailaddr],['your username',$signinfields{'masteruser'}],['your password',$signinfields{'masterpass'}],['log-in at',$baseview.$signinfields{'dest'}] );
my ($gerr,$gmsg) = sign_email_out(\@fs,\@signup,\@bccaddr,$fromaddr,$sub,"ip");
$debug.= "$gmsg <br />";
sign_respond("Problem:",{ 'msg' => "Invalid form information from $ENV{'REMOTE_ADDR'} $gerr $debug" },"errorarea") if $gerr ne ""; 

my @fs1 = ( ['request',$signinfields{'newalert'}],['name',$signin],['email',$emailaddr] );
my ($ferr,$fmsg) = sign_email_out(\@fs1,\@toaddr,\@bccaddr,$fromaddr,$sub,"ip");
$debug.= "$fmsg <br />";
sign_respond("Problem:",{ 'msg' => "Invalid form information from $ENV{'REMOTE_ADDR'} $ferr $debug" },"errorarea") if $ferr ne ""; 
...
###sign_html_out($otxt);
####
sign_respond("error:",{ 'msg' => "Cannot process form: incorrect fields from $ENV{'REMOTE_ADDR'}. Please refresh page and try again. $debug","errorarea" });
}
...
my $f = $base.$signinfields{'loginfile'};
my $s = "";
my $err = undef;
my $signtitle = <<_SIGNTITLE_;
									<ul class="area editablearea signinarea responsearea$cls">       
										<li class="column">
											<div class="row editblock">
           
												<div class="edittext">
													<div class="text">
														<p class="format2">$outhead</p>
														<p>$outtext</p>
													</div>
												</div>
...
my $signin = <<_SIGNIN_;
													<div class="form signin">
														<form id="cgi_form_0" method="post" accept-charset="UTF-8" action="$cgisign"><fieldset>
														<ul class="ful">
														<li class="fli title">Log In</li>
														<li class="fli"><label for="username_0">Username<span class="required"></span></label><input name="pre_username_0" id="username_0" type="text" value="$user"></li>
														<li class="fli"><label for="password_0">Password<span class="required"></span></label><input name="pre_password_0" id="password_0" type="password" value="$pass"></li>
														</ul>
...
my $remindin = <<_SIGNREMIND_;
													<div class="form signremind">
														<form id="cgi_form_1" method="post" accept-charset="UTF-8" action="$cgisign"><fieldset>
														<ul class="ful">
